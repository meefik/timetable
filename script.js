function setDefaultDatas() {
    var from = new Date().set({millisecond: 0, second: 0, minute: 0, hour: 0});
    (from.getDay() !== 1) ? (from.moveToDayOfWeek(1, -1)) : (void(0));
    from.addWeeks(-2);

    var to = new Date().set({millisecond: 0, second: 0, minute: 0, hour: 0});
...
function show() {
    ...
    for (var i = 0; i < document.cdata.length; i++) {
        var courseRaw = document.cdata[i];
    ...
    var result = {"headers" : head, "courses" : courses};
...
function showModalBody(courseId){
    ...
    var result = {"checkitems" : checkItems, "courseId" : courseId};
...
function updateCheckitem(type, checkitemID, state){
    ...
    if (type === "update"){
        ...
        dataa = {"id" : checkitemID, "state" : state}

    } else if (type === "duplicate") {
        urla = "duplicateLastHistory";
        dataa = {"id" : checkitemID};
    }
...
function showCheckItemHistory(eButton, checkItemId){
    ...
    result = {"checkitemHistorys" : result, "checkItemId" : checkItemId};
...
function saveHistoryComment(eButton, historyID){
    ...
    var result = JSON.parse($.ajax({
        ...
        data : {"id" : historyID, "comment" : comment},
        type: "GET"
    }).responseText);
...
function getHead(from, to){
    ...
    while (cur < to) {
        ...
        curSunday.set({millisecond: 99, second: 59, minute: 59, hour: 23});
        if (today.between(cur, curSunday)) {
            current = true;
        }

        dates.push({"date" : date, "current" : current, "year" : year, "monday" : cur.toString("dd.MM.yyyy")});
        cur.addWeeks(1);
    }
...
function getCourseEvent(course, dateOfMonday) {
    ...
    currentSunday.set({millisecond: 99, second: 59, minute: 59, hour: 23});
    var now = new Date();
